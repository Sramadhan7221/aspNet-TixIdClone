...
namespace AspNetFilmApp.Services
{
     public class FilmService : IFilmService
     {
          ...
          public FilmService()
          {
            ...
            new Film("Ambulance(IMAX 2D)","Michael Bay","Jake Gyllenhaal, Yahya Abdul-Mateen II, Eiza Gonzalez, Garret Dillahunt, Devan Chandler Long, A Martinez, Andy Favreau ","Dua perampok membajak mobil ambulance setelah aksi perampokan mereka tidak berjalan sesuai rencana.","136","D17+"," Action, Crime, Drama"){
            ...
            },new Film("Ambulance(3D)","Michael Bay","Jake Gyllenhaal, Yahya Abdul-Mateen II, Eiza Gonzalez, Garret Dillahunt, Devan Chandler Long, A Martinez, Andy Favreau ","Dua perampok membajak mobil ambulance setelah aksi perampokan mereka tidak berjalan sesuai rencana.","136","D17+"," Action, Crime, Drama"){
            ...
            },new Film("Marley","Denny Siregar","Tengku Tezi, Tyas Mirasih, Jason L.Theo. Emmie Lemu, Yadi Timo, Chika Waode Aden Bajaj, Isa Bajaj, Ricky Malau, Roni Gulung","Marley, seekor anjing bersahabat dengan Doni, guru matematika. Satu saat, Doni dipecat dari sekolah karena dia menerapkan sistem belajar yang berbeda. Akhirnya Doni membuka les matematika sendiri. Les yang awalnya sepi, akhirnya ramai. Disanalah Doni bertemu Vina dan muncul rasa suka diantara mereka berdua. Ketegangan terjadi ketika ada lomba matematika, Marley diculik oleh pedagang daging anjing. Bagaimana nasib Marley? Bisakah Doni menyelamatkan sahabatnya itu?","106","SU","Drama"){
          ...
          public int Add(FilmViewModel request)
          {
          ...
          {
               List<FilmViewModel> filmViewModel = new List<FilmViewModel>();
               foreach (var item in _film)
               {
                    filmViewModel.Add(new FilmViewModel(item.Judul, item.Sutradara, item.Aktor, item.Sinopsis, item.Durasi, item.RatingUsia, item.Genre)
                    {
                         LinkImg = item.LinkImg,
                         LinkTrailer = item.LinkTrailer,
                         TglTayang = item.TglTayang
                    });
               }
